...
def test_lasso_opt_oracle_edge_cases():
    ...
    # Тест 2: Очень маленькое t
    t_small = 1e-10
    oracle = LASSOOptOracle(A, b, reg_coef, t_small)
    func_val_small = oracle.func(xu)
    assert func_val_small < 0  # Доминирует барьерный член
...
def test_line_search_tool_edge_cases():
    ...
    # Тест 2: Метод Вульфа на невыпуклой функции
    class NonConvexOracle:
        def func_directional(self, x, d, alpha):
            return -np.abs(alpha - 0.5)  # Минимум в alpha=0.5
            
        def grad_directional(self, x, d, alpha):
            return np.sign(0.5 - alpha)  # Субградиент
    ...
    assert 0.4 < alpha < 0.6  # Должен найти минимум около 0.5
